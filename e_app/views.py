...
def plot_power_source_distribution():
    ...
    power_sources = PowerSource.objects.filter(
        active=True
...
def plot_power_consumption():
    ...
    data = ConsumptionData.objects.filter(power_source=power_source).order_by(
        "start_date"
    )
...
def home(request):
    ...
    total_energy_consumed = (
        power_sources.aggregate(Sum("power_supplied"))["power_supplied__sum"] or 0
    )
    a_total_energy_consumed = (
        consumption_data.aggregate(Sum("power_consumed"))["power_consumed__sum"] or 0
    )
    ...
    solar_total_energy = (
        solar_data.aggregate(Sum("power_consumed"))["power_consumed__sum"] or 0
    )
    grid_total_energy = (
        grid_data.aggregate(Sum("power_consumed"))["power_consumed__sum"] or 0
    )
    cerm_total_energy = (
        cerm_data.aggregate(Sum("power_consumed"))["power_consumed__sum"] or 0
    )
